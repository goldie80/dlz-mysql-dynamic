...
#endif
#ifdef USE_DLZ_DLOPEN_V2
#include <dlz/dlz_dlopen_v2.h>
#endif

#include <dlz/isc_list.h>
...
sdlzh_build_querylist(dbinstance_t *dbi, const char *query_str, char **zone,
  ...
  tql = malloc(sizeof(query_list_t));
  ...
  while (right_str != NULL) {
    ...
    tseg = malloc(sizeof(query_segment_t));
     ...
     * left portion
     */
    tseg->sql = strdup(isc_string_separate(&right_str,
                                           "$"));
    if (tseg->sql == NULL) {
...
sdlzh_build_sqldbinstance(const char *allnodes_str,
  ...
  if (dbi->putnamedrr != NULL) {
    db->putnamedrr = dbi->putnamedrr;
  }
  if (dbi->writeable_zone != NULL) {
    db->writeable_zone = dbi->writeable_zone;
  }
  
  /* build the all nodes query list */
  result = sdlzh_build_querylist(dbi, allnodes_str, &db->zone,
                                 &db->record, &db->client,
  ...
  result = sdlzh_build_querylist(dbi, allowxfr_str, &db->zone,
                                 &db->record, &db->client,
                                 &db->allowxfr_q,
  ...
  result = sdlzh_build_querylist(dbi, authority_str, &db->zone,
                                 &db->record, &db->client,
  ...
  result = sdlzh_build_querylist(dbi, findzone_str, &db->zone,
                                 &db->record, &db->client,
  ...
  result = sdlzh_build_querylist(dbi, countzone_str, &db->zone,
                                 &db->record, &db->client,
  ...
  result = sdlzh_build_querylist(dbi, lookup_str, &db->zone,
                                 &db->record, &db->client,
 ...
 cleanup:
  ...
  sdlzh_destroy_sqldbinstance(db);
  /* return failure */
  return (ISC_R_FAILURE);
}
...
sdlzh_destroy_sqldbinstance(dbinstance_t *dbi) {
  ...
  sdlzh_destroy_querylist(&dbi->allnodes_q);
  sdlzh_destroy_querylist(&dbi->allowxfr_q);
  sdlzh_destroy_querylist(&dbi->authority_q);
  sdlzh_destroy_querylist(&dbi->findzone_q);
  sdlzh_destroy_querylist(&dbi->countzone_q);
  sdlzh_destroy_querylist(&dbi->lookup_q);
